...
export function HeaderNav() {
  ...
  const selectedKeys = [items.find(i => pathname.startsWith(i.key))?.key || 'home']

  return (
    <header style={{ background:'#fff', borderBottom:'1px solid var(--border)' }}>
      <div className="container" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', height:64 }}>
        {/* ЛОГО */}
        <Link to="/" aria-label="DANCE POINT — на главную" className="brand" style={{ height: 'inherit'}}>
          <img src={logoUrl} alt="DANCE POINT" className="brand__img" />
        </Link>

        {/* Меню десктоп */}
        <div className="menu_desktop only-desktop">
          <Menu mode="horizontal" selectedKeys={selectedKeys} items={items} disabledOverflow/>
        </div>

        
        <Switch checkedChildren="PL" unCheckedChildren="EN" defaultChecked style={{ marginRight: '12px'}} />

        <div className="only-desktop">
          <Button type="primary"><Link to="/contacts">Trial lesson</Link></Button>
        </div>

        {/* Бургер мобилка */}
        <Button className="only-mobile" type="text" icon={<MenuOutlined />} onClick={()=>setOpen(true)} aria-label="Открыть меню" />
      </div>

      <Drawer width="80%" open={open} onClose={()=>setOpen(false)} title="Menu">
        <Menu mode="inline" selectedKeys={selectedKeys} items={items} onClick={()=>setOpen(false)} />
